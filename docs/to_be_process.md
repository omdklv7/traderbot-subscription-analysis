...
> **Примечание по нотации:** диаграмма используется как аналитическая BPMN 2.0-модель и не предназначена для исполнения на конкретном движке. Отдельные технические параметры элементов в исходном файле используются только для корректного моделирования в редакторе и не описывают фактическую программную реализацию TraderBot.
